<form method="post" id="form-detail" action="{{ route('spbe.update') }}" enctype="multipart/form-data">
    @csrf
    <div class="row">
        @foreach ($data as $data)
            ...
            <div class="col-sm">
                ...
                <table class="table">
                    ...
                    @foreach ($data->jawabans as $a)
                        <tbody>
                            @forelse  ($a->jawabanskpd->where('user_id', auth()->user()->id) as $c)
                                {{-- @foreach ($data->jawabans as $a) --}}
                                <tr style="vertical-align: middle;">
                                    <td>
                                        <div class="form-check mt-3">
                                            <input style="height:35px; width:35px;vertical-align: middle;"
                                                name="jawaban_skpd_id" class="form-check-input" type="radio"
                ...
                {{-- <div class="form-check mt-3">
                            <input name="jawaban_skpd_id" class="form-check-input" type="radio"
                                value="{{ $a->id }}" id="defaultRadio1" checked>
            ...
            <div class="col-md-12 mt-4">
                ...
                <table class="table" id="dynamic_field_lampiran">
                    @foreach ($data->trxuploads->where('user_id', auth()->user()->id) as $b)
                        <tr>
                            <td>
                            ...
                            <td>
                                <a onclick="deletebtn('{{ $b->id }}')" class="btn btn-sm btn-flat btn-danger"><i
                    ...
                    <tr>
                        <td>
                            <input type="file" id="file" name="file[]" class="form-control file_list">
                        </td>
            ...
            <div class="col-md-12">
                <small class="text-light fw-semibold">Keterangan :</small>
                @foreach ($keterangan as $keterangan)
                    <textarea name="keterangan" id="keterangan" cols="10" rows="10" class="form-control">{{ $keterangan->keterangan }}</textarea>
                @endforeach
            </div>
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        url: window.location.origin + '/skpd/spbe/deleteupload',
                        method: "POST",
                        headers: {
