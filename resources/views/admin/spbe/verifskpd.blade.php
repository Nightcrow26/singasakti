<form method="post" id="form-detail-admin" action="{{ route('admin.spbe.update') }}" enctype="multipart/form-data">
    @csrf
    <div class="row">
        @foreach ($trx as $trx)
...
<script>
    ...
    $('#form-detail-admin').on('submit', function(e) {
        e.preventDefault()

        $("#form-detail-admin").ajaxSubmit({
            beforeSend: function() {
                $('#tombol').hide();
                $('#loading').show();
            },
            success: function(res) {
                ...
                if (res.status == "failed") {
                    ...
                    $("#form-detail-admin input:not([name='_token']").val('')
                    $("#modal-create-skpd").modal('hide')
                    $('#tombol').show();
                    $('#loading').hide();
                }
